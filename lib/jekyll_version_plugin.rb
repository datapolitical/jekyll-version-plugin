    ...
    class Tag < Liquid::Tag
      ...
      def git_describe
        tagged_version = case params.fetch(:format, "short")
                         when "short", OPTION_NOT_SPECIFIED
                           run("git  -C $(bundle info contrast --path) describe --tags --always")
                         when "long"
                           run("git  -C $(bundle info contrast --path) describe --tags --always --long")
                         end
      ...
      def parse_head
        head_commitish = case params.fetch(:format, "short")
                         when "short", OPTION_NOT_SPECIFIED
                           run("git  -C $(bundle info contrast --path) rev-parse --short HEAD")
                         when "long"
                           run("git  -C $(bundle info contrast --path) rev-parse HEAD")
                         end
